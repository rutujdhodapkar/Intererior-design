...
try:
    ...
    from backend.image_generator import generate_image_from_prompt
...
def send_message():
    ...
    try:
        # Run reasoning model first
        validation = validate_request(text)
        is_valid = validation.get("is_valid", False)
        # Save reasoning output for debugging
        REASONING_FILE = os.path.join(DATA_DIR, "reasoning_output.json")
        save_json(REASONING_FILE, validation)

        if not is_valid:
            bot_text = "Invalid"
            bot_images = []
        else:
            refined_text = validation.get("refined_prompt", text)
            rooms_count = validation.get("total_rooms")
            dims = validation.get("house_dimensions")
            furniture = ", ".join(validation.get("interior_furniture", []))
            # Build rich details suffix
            rich_details = ""
            if dims:
                rich_details += f", dimensions: {dims}"
            if furniture:
                rich_details += f", featuring furniture: {furniture}"
            if rooms_count:
                rich_details += f", total rooms: {rooms_count}"
            # Generate exterior image
            ext_prompt = f"Exterior architecture of {refined_text}{rich_details}, photorealistic 8k render"
            try:
                url_ext = generate_image_from_prompt(ext_prompt)
                bot_images.append(url_ext)
            except Exception as e:
                print(f"Error generating exterior: {e}")
            # Generate floor plan image
            plan_prompt = f"Architectural floor plan of {refined_text}{rich_details}, top‑down 2D layout, labeled rooms"
            try:
                url_plan = generate_image_from_prompt(plan_prompt)
                bot_images.append(url_plan)
            except Exception as e:
                print(f"Error generating floor plan: {e}")
            # Generate interior room images (use a default set if validator doesn't provide specific rooms)
            default_rooms = ["Living Room", "Kitchen", "Bedroom", "Bathroom"]
            for room in default_rooms:
                room_prompt = f"Professional interior design of {refined_text} {room}{rich_details}, award‑winning 2D design, furnished, high‑quality render"
                try:
                    url_room = generate_image_from_prompt(room_prompt)
                    bot_images.append(url_room)
                except Exception as e:
                    print(f"Error generating {room}: {e}")
            bot_text = f"Generated design suite for your house project: {refined_text}"
    except Exception as e:
